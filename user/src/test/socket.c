/*
 * AF_PACKET/SOCK_RAW socket smoke test.
 *
 * Same exchange as the /dev/eth0 test (eth.c) but through the socket
 * interface: sends an ARP request for the qemu user-mode network
 * gateway (10.0.2.2) and waits for the reply.
 */
...
int main(void)
{
    ...
    n = send(fd, arp_req, sizeof(arp_req), 0);
    if (n != sizeof(arp_req)) {
        printf("error: arp request send failure (%d)\n", (int)n);
        return 1;
    }
